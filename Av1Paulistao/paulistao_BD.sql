...
AS
	...
	WHERE grupo = SUBSTRING(@grupo,2,1)
		...
		INSERT INTO jogos
		SELECT DISTINCT timeA.CodigoA, timeB.CodigoB, FLOOR(RAND() * 5 + 1), FLOOR(RAND() * 5 + 1), NULL
...
SELECT tA.nome_time AS timeA, tB.nome_time AS timeB, j.gols_timeA, j.gols_timeB, j.data_jogo
